//无返回值的参数
function myFirstTypeScriptFunction() {
    console.log("调用函数");
    return;
}
myFirstTypeScriptFunction();
//函数返回值的应用
function mySecondTypeScriptFunction() {
    return "return a string";
}
function callFunctionTest() {
    var functionString = mySecondTypeScriptFunction();
...
//可选参数和默认参数
function buildNameX(firstName, lastName) {
    if (lastName)
        return firstName + " " + lastName;
    else
        return firstName;
}
var result1 = buildNameX("Bob"); // 正确
var result3 = buildNameX("Bob", "Adams"); // 正确
...
console.log(x);
var myConF = new Function("x", "y", "z", "return x / y * z");
var myConFC = myConF(100, 2, 50);
console.log(myConFC);
//递归函数
//递归函数即在函数内调用函数本身
...
//Lambda 函数也称之为箭头函数。
// 箭头函数表达式的语法比函数表达式更短。
var arrowFunction = function (x) { return 10 + x; }; //输入参数为x, return 10+x;
console.log(arrowFunction(100)); //输出结果为 110
//我们可以不指定函数的参数类型，通过函数内来推断参数类型
var arrowFunctionSecond = function (x) {
    if (typeof x == "number") {
        console.log((x + "是一个数字"));
    }
